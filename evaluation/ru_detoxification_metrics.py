...
def evaluate_cosine_similarity(
    ...
    a = encode_cls(original_texts, model=model, tokenizer=tokenizer, batch_size=batch_size, verbose=verbose)
    b = encode_cls(rewritten_texts, model=model, tokenizer=tokenizer, batch_size=batch_size, verbose=verbose)
    
    scores = torch.einsum('ij, kj -> ikj', torch.tensor(a), torch.tensor(b)).reshape(a.shape[0]*b.shape[0], a.shape[1]).sum(1)
